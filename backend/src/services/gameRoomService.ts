...
export class GameRoomService {
  ...
  async startGame(roomId: string, hostId: string): Promise<Game> {
    ...
    activeGameStore.setGame(game.id, game);
  ...
  // Crear juego desde sala (deck, deal, initial state via gameLogicService)
  private async createGameFromRoom(room: GameRoom): Promise<Game> {
    const tilesPerPlayer = room.settings.initialTiles;
    const playersForDeal: Player[] = room.players.map((p) => ({
      ...p,
      tiles: [],
      hasMadeInitialMeld: false,
    }));
    const pool = createDeck();
    const { pool: remainingPool, players: dealtPlayers } = deal(pool, playersForDeal, tilesPerPlayer);

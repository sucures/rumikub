...
export async function purchaseItem(itemId: string, amount = 1): Promise<{ success: boolean; balance: { coins: number; gems: number } }> {
  const { data } = await apiClient.post<{ success: boolean; balance: { coins: number; gems: number }; error?: string }>(
    `/api/store/purchase/${itemId}`,
    { amount }
  );
